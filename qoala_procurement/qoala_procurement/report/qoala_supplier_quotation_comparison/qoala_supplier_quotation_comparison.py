...
def get_data(filters):
	...
	if filters.get("item_code"):
		query = query.where(sq_item.item_code.isin(filters.get("item_code")))
...
def prepare_data(supplier_quotation_data, filters):
	...
	if filters.get("item_code"):
		...
		chart_data = prepare_chart_data(suppliers, qty_list, supplier_qty_price_map, filters.get("item_code"))

	return out, chart_data


def prepare_chart_data(suppliers, qty_list, supplier_qty_price_map, item_codes):
    data_points_map = {item_code: {} for item_code in item_codes}
    qty_list.sort()

    # Create qty wise values map for each item code
    for item_code in item_codes:
        for supplier in suppliers:
            entry = supplier_qty_price_map[supplier]
            for qty in qty_list:
                if qty not in data_points_map[item_code]:
                    data_points_map[item_code][qty] = []
                if qty in entry:
                    data_points_map[item_code][qty].append(entry[qty])
                else:
                    data_points_map[item_code][qty].append(None)
    ...
    for item_code in item_codes:
        for qty in qty_list:
            datapoints = {
                "name": f"{item_code} {currency_symbol} (Qty {qty})",
                "values": data_points_map[item_code][qty],
            }
            datasets.append(datapoints)

    chart_data = {"data": {"labels": suppliers, "datasets": datasets}, "type": "bar"}
